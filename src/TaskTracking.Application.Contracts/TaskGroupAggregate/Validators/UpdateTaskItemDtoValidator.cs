...
public class UpdateTaskItemDtoValidator : AbstractValidator<UpdateTaskItemDto>
{
    ...
    public UpdateTaskItemDtoValidator(IStringLocalizer<TaskTrackingResource> localizer)
    {
        ...
        RuleFor(x => x.RecurrencePattern)
            ...
            .WithMessage(_localizer[global::TaskTracking.TaskTrackingDomainErrorCodes.RecurrencePatternRequired]);

        RuleFor(x => x.RecurrencePattern)
            .Null()
            .When(x => x.TaskType == TaskType.OneTime)
            .WithName(_localizer["Recurrence"])
            .WithMessage(_localizer[global::TaskTracking.TaskTrackingDomainErrorCodes.CannotSetRecurrencePatternForOneTimeTask]);
